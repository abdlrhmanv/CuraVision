...
export default function PatientDashboard() {
  ...
  const stats = [
    { value: '7', label: 'Scans', icon: Brain },
    { value: '5', label: 'Reports', icon: FileText },
    { value: '12', label: 'Articles', icon: Newspaper },
    { value: '4.8', label: 'Avg Rating', icon: Star },
  ]

  const recentReports = [
    { scanName: 'Brain MRI - Axial T1', note: 'No obvious mass lesion detected.', status: 'Confirmed', statusColor: 'green' },
    { scanName: 'CT Scan - Coronal', note: 'Possible early signs of sinusitis.', status: 'In Review', statusColor: 'blue' },
    { scanName: 'Brain MRI - Sagittal T2', note: 'Please upload higher resolution images.', status: 'Declined', statusColor: 'warn' },
  ]
    ...
    <>
      ...
      <div className="bg-gradient-to-r from-accent/10 via-blue/5 to-purple/5 border border-border rounded-2xl p-5 md:p-7 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">Good morning, Omar 👋</h1>
          <p className="text-xs text-muted mt-1.5">Tuesday, April 20, 2026 — Your health journey with CuraVision</p>
        </div>
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        <div className="text-[10px] tracking-[2px] uppercase text-muted font-semibold mb-3 flex items-center gap-2">
          <FileText size={12} /> Recent Reports Summary
        </div>
        <div className="space-y-2">
          {recentReports.map((report) => (
            <div key={report.scanName} className="bg-surface border border-border rounded-lg p-3.5 flex items-center justify-between gap-3 flex-wrap">
              <div className="flex-1">
                <div className="text-sm font-semibold">{report.scanName}</div>
                <div className="text-xs text-muted mt-1">{report.note.substring(0, 80)}...</div>
              </div>
              <span className={`text-[10px] px-2 py-1 rounded ${
                report.statusColor === 'green' ? 'bg-green/15 text-green' :
                report.statusColor === 'blue' ? 'bg-blue/15 text-blue' : 'bg-warn/15 text-warn'
              }`}>
                {report.status}
              </span>
            </div>
